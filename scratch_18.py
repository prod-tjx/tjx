...
for notification in notifications:
    ...
    run_data.append({
        '#': entry_num,
        'Date/Time': f"{date_str}, {time_str}",
        'TH': th,
        'GCT': gct,
        'Total': total,
        'Runtime (s)': runtime,
        'Queries Active': ', '.join(queries),
        'Est. TFA Value': est_tfa_value
    })
...
# Format Est. TFA Value as currency string for display
df_runs['Est. TFA Value'] = df_runs['Est. TFA Value'].apply(lambda x: f"${x:,.2f}")
...
fiscal_data = {
    'Metric': [
        ...
        'Estimated TFA Value (TH × Avg Value)',
        '',
        'DISCLAIMER',
