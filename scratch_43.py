...
def create_temp_table():
    """Create temp table with FF orders excluded via APPLE_MAPS_ADD join"""
    print("Creating temporary table...")

    sf.run_query("DROP TABLE IF EXISTS gbi_fraud_bap_db.ai_live_biz_app.trader_temp_frame")

    # FF diagnostic — runs before CREATE so we can see what gets filtered
    diag = sf.run_query("""
        SELECT
            COUNT(*) AS total,
    ...
    print(f"✅ Temp table created (FF excluded) in {duration:.2f}s")
    return duration
    ...
    try:
        ...
        if all_results:
            ...
            if success:
                ...
                print(f"⏱️ Breakdown: Temp({temp_creation_time:.1f}s) + Patterns({total_pattern_time:.1f}s) + Write({write_time:.1f}s) = Total({total_runtime:.1f}s)")
