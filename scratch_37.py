"""
Validate GitHub active_queries/ against live Snowflake DDLs.

Reports:
  - In Snowflake but missing from repo       (ADD to repo)
  - In repo but not active in Snowflake      (REMOVE from repo OR reactivate in SF)
  - Present in both but DDL differs          (DRIFT — decide source of truth)
  - Present in both and identical            (OK)

Modes (set MODE below):
  "check"     = read-only report, no file changes
  "pull"      = repo becomes mirror of Snowflake (overwrites + removes stale files)
  "interactive" = prompt for each drift (skip / pull from SF)
...
MODE       = "check"   # "check" | "pull" | "interactive"
# ============================================================================
...
def normalize(ddl: str) -> str:
    """Strip comments + whitespace + case for semantic comparison."""
    if not ddl:
        return ""
    ...
    # Collapse whitespace, uppercase, strip trailing semicolon
    return re.sub(r"\s+", " ", ddl.strip().upper()).rstrip(";").strip()


def read_repo_ddl(path: Path) -> str:
    """Read a repo .sql file, stripping our auto-generated header comments."""
    if not path.exists():
        return ""
...
def repo_name_for(table_name: str, existing_files: set[str]) -> str:
    """Find matching repo filename case-insensitively, else default to TABLE_NAME.sql."""
...
def run():
    if not ACTIVE_DIR.exists():
        print(f"❌ {ACTIVE_DIR} does not exist. Did you clone the repo?")
        return
    ...
    repo_stems = {f[:-4] for f in repo_files}  # strip .sql
    ...
    if MODE == "interactive":
        for name in only_in_sf:
            ans = input(f"\n➕ Add {name} to repo? [y/N] ").strip().lower()
            if ans == "y":
                ddl = get_ddl(name)
                if ddl:
                    (ACTIVE_DIR / f"{name}.sql").write_text(format_file(name, ddl), encoding="utf-8")
                    print(f"     wrote {name}.sql")
        for name, sf_ddl, path in drifted:
            ans = input(f"\n⚠️  Update {path.name} from Snowflake? [y/N] ").strip().lower()
            if ans == "y":
                path.write_text(format_file(name, sf_ddl), encoding="utf-8")
                print(f"     updated {path.name}")
        for name in only_in_repo:
            ans = input(f"\n➖ Delete {name}.sql from repo? [y/N] ").strip().lower()
            if ans == "y":
                fname = repo_name_for(name, repo_files)
                (ACTIVE_DIR / fname).unlink()
                print(f"     deleted {fname}")
