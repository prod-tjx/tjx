...
# Split by date headers (Jan 5th at 09:37 format)
...
for i, notification in enumerate(notifications):
    if not notification.strip():
        skipped.append(f"Chunk {i}: Empty")
        continue

    if 'Trader Alerts Results' not in notification:
        skipped.append(f"Chunk {i}: No 'Trader Alerts Results' found")
        continue

    # Extract date and time (Jan 5th at 09:37 format)
    datetime_match = re.search(r'(\w+ \d+)\w* at (\d{2}:\d{2})', notification)
    if not datetime_match:
        skipped.append(f"Chunk {i}: No date/time match. First 100 chars: {notification[:100]}")
        continue
    ...
    if not th_match:
        skipped.append(f"Chunk {i}: No TH value found")
        continue
