...
# ─────────────────────────────────────────────────────────────────────────────
# Query 1 + 2: Temp table with FF exclusion baked in
# ─────────────────────────────────────────────────────────────────────────────

def create_temp_table() -> float:
    """
    Builds trader_temp_frame from last 5 days.
    FF orders excluded here once — all downstream patterns inherit the filter.
    Join: A1_ADD vs tfa_aso_freight_forwarder_address_log.
    If address matches → known FF address → excluded before any pattern runs.
    """
    # Query 1
    ...
    sql = """
        ...
        LEFT JOIN gbi_fraud_bap_db.ai_live_biz_app.tfa_aso_freight_forwarder_address_log ff
            ON ff.output_apple_maps_normalized_address_unit_stripped = tf.A1_ADD
...
# ─────────────────────────────────────────────────────────────────────────────
# Query 4: Pattern execution — FF already excluded via temp table
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    try:
        ...
        if not result.empty:
            preview_cols = [
                'WEB_ORDER_ID', 'A1_ADD', 'LATEST_A1_ADD',
                'B2_EM', 'A1_EM', 'TOTAL_VALUE',
            ]
