...
def execute_actions(action_df: pd.DataFrame) -> None:
    """Send the action dataframe to the ACM API."""
    if action_df.empty:
        log.info("Nothing to action — dataframe is empty.")
        return

    confirm_count = (action_df["action"] == "TRADERCONFIRM").sum()
    release_count = (action_df["action"] == "TRADERRELEASE").sum()
    log.info(
        f"Actioning {len(action_df)} orders  "
        f"(CONFIRM: {confirm_count} | RELEASE: {release_count})"
    )

    acm = ACMTraderHelper(env=ACM_ENV)
    acm.df_for_action(action_df)
    acm.action_cases()

    log.info(f"ACM mass-action key: {acm.mass_action_key}")
...
def run_once() -> None:
    """Full pipeline: fetch → evaluate → action."""
    start = datetime.now()
